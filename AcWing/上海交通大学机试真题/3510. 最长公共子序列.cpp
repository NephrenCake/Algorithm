...
const int N = 1000010;
int n, a[N], b[N];
int q[N], len;  // q[i]长度为i的序列中结尾的最小值
...
int main() {
    ...
    for (int i = 1, t; i <= n; i++) cin >> t, b[i] = a[t];  // b存的是原数组对应a中元素的下标，转化为b的最长上升子序列

    for (int i = 1; i <= n; i++) {
        if (b[i] == 0) continue;
        int l = 0, r = len, m, tar = b[i];  // 找值b[i]能接在多长的子序列后面
        while (l < r) {
            m = (l + r + 1) / 2;
            if (q[m] < tar) l = m;
            else r = m - 1;
...
// int main() {
//     cin >> n;
//     for (int i = 1, t; i <= n; i++) cin >> t, a[t] = i;  // 交换值和下标。则两者的公共子序列，必定满足a[b[k]]严格单调递增
//     for (int i = 1, t; i <= n; i++) cin >> t, b[i] = a[t];  // b存的是原数组对应a中元素的下标，转化为b的最长上升子序列
//
//     for (int i = 1; i <= n; i++) {  // b中前i个数字并且以b[i]结尾的最长上升的子序列
//         if (b[i] == 0) continue;  // 注意！如果为0则表示不存在，则以b[i]结尾的最长上升的子序列也就不存在
//         f[i] = 1;
//         for (int j = 1; j < i; j++)
//             if (b[j] < b[i])
